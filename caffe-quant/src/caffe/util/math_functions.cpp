...
template <typename Dtype>
void fixpoint_fake_quantize_cpu(Dtype* data, int data_count, Dtype threshold, QuantType dtype) {
  double inv_scale = 0;
  double scale = 0;
  int upper = INT_MAX;
  int lower = INT_MIN;
...
template void fixpoint_fake_quantize_cpu<>(int* data, int data_count, int threshold, QuantType dtype);
template void fixpoint_fake_quantize_cpu<>(unsigned int* data, int data_count, unsigned int threshold, QuantType dtype);
template void fixpoint_fake_quantize_cpu<>(float* data, int data_count, float threshold, QuantType dtype);
template void fixpoint_fake_quantize_cpu<>(double* data, int data_count, double threshold, QuantType dtype);
template void fixpoint_fake_quantize_cpu<>(bool* data, int data_count, bool threshold, QuantType dtype);
